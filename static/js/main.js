...
let vue = new Vue({
    ...
    methods: {
        ...
        setBackgroundLazy(entries, observer, isIntersecting) {
            ...
            if (isIntersecting) {
                if (!!aniBg) {
                    ...
                    entries[0].target.attributes.style.value = 'background-image:url(' + bg + '),url(' + cdn2 + aniBg + '&af&l=6&output=webp' + ');background-blend-mode: luminosity;background-size:100% 100%;'
                } else {
        ...
        chtGen() {
            return ['裏番(SFW)', '泡麵番', '動作', '成年人主角', '冒險', '擬人化', '前衛', '曾經得獎', '耽美', '純女角', '育兒', '格鬥運動', '喜劇', '變裝', '不良', '偵探', '劇情', '賣肉', '教育', '成人耽美', '奇幻', '惡作劇幽默', '百合', '血腥', '美食', '後宮', '高風險遊戲', '歷史', '恐怖', '偶像(女性)', '偶像(男性)', '異世界', '療癒', '多角戀', '性轉', '魔法少女', '武術', '機甲', '醫療', '軍事', '音樂', '神秘', '神話', '組織犯罪', '宅圈', '惡搞', '表演藝術', '寵物', '心理', '競速', '異世界重生', '乙女', '浪漫', '浪漫敘事', '武士', '學園', '科幻', '娛樂圈', '空氣系', '太空', '體育', '策略', '超能力', '超自然力量', '生存', '懸疑', '團隊運動', '時間旅行', '吸血鬼', '電玩', '視覺藝術', '職場']
        },
        simpPYStr() {
